<div align="center">

<img height="120" src="https://media.giphy.com/media/M9gbBd9nbDrOTu1Mqx/giphy.gif" />

# Hi, I'm Abhishek 👋

**AI/ML Engineer · Full-Stack Developer · Generative AI Enthusiast** 

Building intelligent systems that solve real-world problems —
from RAG pipelines to multi-agent architectures.

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mishraabhishek24)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=flat&logo=gmail&logoColor=white)](mailto:abhishekmishra24112004@gmail.com)
[![WhatsApp](https://img.shields.io/badge/WhatsApp-25D366?style=flat&logo=whatsapp&logoColor=white)](https://wa.me/qr/HB2NKO6HBOJWB1)
[![Discord](https://img.shields.io/badge/Discord-7289DA?style=flat&logo=discord&logoColor=white)](https://discord.com/users/abhishekmishra2004)

![Visitor Badge](https://visitor-badge.laobi.icu/badge?page_id=Abhishekmishra20041123.Abhishekmishra20041123)

</div>

---

## About

I'm passionate about turning research-grade AI into production-ready
products. My work sits at the intersection of LLMs, full-stack
engineering, and real-world problem-solving — with a focus on
healthcare, business intelligence, and automation.

---

## What I'm building

| Area | Focus |
|---|---|
| 🤖 AI/ML | RAG applications, LangGraph multi-agent systems |
| 🌐 Full-Stack | React, Next.js, FastAPI, Node.js |
| ☁️ Infra | Docker, MLOps, Cloud-native architectures |
| 🏥 Domains | Healthcare AI, Business Intelligence, Automation |

---

## What I'm learning

- Advanced RAG, agentic workflows, and LLM fine-tuning
- LangGraph and large-scale agent orchestration
- Kubernetes, MLOps, and distributed system design

---

## Achievements

| | |
|---|---|
| 🥇 | Hackathon Finalist — **Incuverse 1.0** |
| 🏅 | Top 10 Finalist — **Enigma 4.0** |
| 🚀 | 5+ AI/ML hackathons and innovation challenges |
| ⚙️ | Multiple production-ready AI systems shipped |

---
<img width="1440" height="1012" alt="image" src="https://github.com/user-attachments/assets/29bb9cbb-a6e9-481c-bb5f-6ded167a359c" />






## 📊 GitHub Stats

<div align="center">

<table>
  <tr>
    <td align="center"><b>📈 Activity Stats</b></td>
    <td align="center"><b>🔥 Contribution Streak</b></td>
  </tr>
  <tr>
    <td>
      <img src="https://raw.githubusercontent.com/Abhishekmishra20041123/Abhishekmishra20041123/stats-output/stats.svg" height="180" />
    </td>
    <td>
      <img src="https://streak-stats.demolab.com?user=Abhishekmishra20041123&theme=gruvbox&hide_border=true&border_radius=8&mode=daily" height="180" />
    </td>
  </tr>
  <tr>
    <td align="center"><b>💻 Top Languages</b></td>
    <td align="center"><b>🏆 Trophies</b></td>
  </tr>
  <tr>
    <td>
      <img src="https://raw.githubusercontent.com/Abhishekmishra20041123/Abhishekmishra20041123/languages-output/languages.svg" height="160" />
    </td>
    <td>
      <img src="https://raw.githubusercontent.com/Abhishekmishra20041123/Abhishekmishra20041123/trophy-output/trophy.svg" height="160" />
    </td>
  </tr>
  <tr>
    <td colspan="2" align="center"><b>📅 Activity Graph</b></td>
  </tr>
  <tr>
    <td colspan="2" align="center">
      <img src="https://raw.githubusercontent.com/Abhishekmishra20041123/Abhishekmishra20041123/activity-graph-output/activity-graph.svg" width="95%" />
    </td>
  </tr>
</table>

</div>

---

## Let's connect

I'm always open to collaborating on open-source AI projects,
hackathons, and startup ideas. If you're working on something
interesting in generative AI or full-stack — reach out.

> *"Continuous learning combined with consistent execution
> drives meaningful innovation."*

---

<div align="center">
<img src="https://raw.githubusercontent.com/Abhishekmishra20041123/Abhishekmishra20041123/snake-output/snake.svg" alt="Snake animation" />
</div>
